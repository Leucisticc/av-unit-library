...
TAG_RULES = {
    ...
    # -- Buffs --------------------------------------------------------------------
    "dmg-buff":   {"group": "buffs", "label": "Damage buff", "icon": "stat_Damage.png", "patterns": [_BUFF + r" (?:the )?(?:[\w'% ]+?)?(?:damage|attack)\b" + _CONDITIONAL, r"\bdamage buff\b", r"\bincreas(?:e|es|ing) (?:the |this unit'?s |its |own )?(?:dot |cosmic |fire |curse |spark |blast |holy |water |nature |passion |unbound )?damage (?:of|by)\b" + _CONDITIONAL, r"\bgains? (?:a )?\+?\d+% damage\b", r"(?<!crit )(?<!burn )\b(?:damage|dmg) \+\d+%", r"\+\d+% attack\b", r"(?<!crit )(?<!burn )(?<!bleed )(?<!dot )\bDMG \+\d+%", r"\+\d+% damage\b", r"\braises? damage by\b"]},
    "range-buff": {"group": "buffs", "label": "Range buff",  "icon": "stat_Range.png", "patterns": [_BUFF + r" (?:this unit'?s |their |its |own |the )?range\b", _BUFF + r" (?:that |the )?(?:ally|unit|allies|units)(?: in range)? by [\w'% ]{0,25}?\brange\b", r"\brange \+\d+%", r"(?<!in )\brange by \d", r"\b(?:and|&) (?:\+?\d+% )?range\b", r"\bRNG \+\d+%", r"\+\d+% range\b", r"\brange buff\b", r"\bincreases? range by\b", r"\bgains? \+?\d+% range\b"]},
    "spa-buff":   {"group": "buffs", "label": "SPA buff",    "icon": "stat_SPA.png", "patterns": [_BUFF + r" (?:the )?(?:[\w' ]+?)?(?:spa|attack speed)\b", r"\b(?:reduc|decreas|lower)(?:e|es|ed|ing)? (?:the |this unit'?s |their |own |its )?spa\b", r"\bSPA -\d+%", r"-\d+% spa\b", r"\bloses? \d+% spa\b"]},
    "crit-buff":  {"group": "buffs", "label": "Crit buff",   "icon": "stat_CRIT.png", "patterns": [_BUFF + r" (?:the )?(?:[\w' ]+?)?crit(?:ical)?(?: rate| damage| chance| dmg)?\b", r"\bcrit(?:ical)? (?:rate|damage|chance|dmg) \+\d+%", r"\+\d+% crit", r"\bgains? \+?\d+% crit", r"\bhas \d+% crit", r"\bovercrit\b"]},
    "cost-reduction": {"group": "buffs", "label": "Cost reduction", "icon": "stat_Cost_Reduce.png", "patterns": [r"\b(?:reduc|lower|decreas)(?:e|es|ed|ing)? (?:the )?(?:[\w' ]{0,25}?)(?:placement |upgrades? |deployment )?(?<!active )(?<!ability )(?<!mana )(?<!meter )costs?\b", r"\bcheaper\b", r"\bcost reduction\b", r"\bcosts? nothing\b", r"\bplace[sd]?[^.;]{0,30}? for free\b", r"\bcosts \d+% less\b", r"\bcosts? (?:are )?reduced\b"]},
    ...
    "income":     {"group": "buffs", "label": "Increase income", "icon": "stat_Farm1.png", "patterns": [r"\byen\b", r"\bincome\b", r"\bmoney\b"]},
    "stocks":     {"group": "mech", "label": "Life stocks", "icon": None, "patterns": [r"\b(?:life|health) ?stocks?\b", r"\blifestocks?\b", r"\b(?:loses?|lost|missing|remove|removes|restores?|reaches?) (?:a |an |one |\d+ )?stocks?\b", r"\bstocks? (?:missing|lost|removed)\b"]},
